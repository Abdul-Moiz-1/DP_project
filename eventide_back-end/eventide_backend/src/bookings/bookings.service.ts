...
export class BookingsService {
  ...
  async getAttendeeStats(userId: number) {
    ...
    return {
      ...
      upcomingEvents: confirmed.filter(b => new Date(b.event.startDate) > now).length,
      pastEvents: confirmed.filter(b => new Date(b.event.endDate) < now).length,
      totalSpent: parseFloat(
        confirmed.reduce((sum, b) => sum + Number(b.ticket.price), 0).toFixed(2),
      ),
