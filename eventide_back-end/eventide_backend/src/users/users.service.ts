...
// }
    ...
    async updateOrganizerProfile(userId: number, dto: Partial<CreateOrganizerProfileDto>) {
        if (!userId) throw new BadRequestException('Invalid user ID');
        ...
        if (!user) throw new NotFoundException('User not found');
        if (!user.organizerProfile) throw new BadRequestException('No organizer profile found');

        Object.assign(user.organizerProfile, dto);
        await this.orgRepo.save(user.organizerProfile);
